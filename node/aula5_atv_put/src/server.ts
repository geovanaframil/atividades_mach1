...
function verifyBody(body: any): { isValid: boolean; mensage: string } {
  ...
  if (!body.productName || typeof body.productName !== "string") {
    return {
      isValid: false,
      mensage: "O campo product name é obrigatorio e deve receber uma string",
    };
  }
  if (!body.productDescription || typeof body.productDescription !== "string") {
    return {
      isValid: false,
      mensage:
        "O campo product description é obrigatorio e deve receber uma string",
    };
  }
  if (!body.productCategory || typeof body.productCategory !== "string") {
    return {
      isValid: false,
      mensage:
        "O campo product category é obrigatorio e deve receber uma string",
    };
  }
  if (!body.productCost || typeof body.productCost !== "number") {
    return {
      isValid: false,
      mensage: "O campo product cost é obrigatorio e deve receber um numero",
    };
  }
  if (
    !body.productTags ||
    !body.productTags.every(
      (productRelated: string) => typeof productRelated === "string"
    )
  ) {
    return {
      isValid: false,
      mensage: " O campo product tags é obrigatorio e deve ser um array",
    };
  }
  if (
    !body.productRelated ||
    !body.productRelated.every(
      (productRelated: number) => typeof productRelated === "number"
    )
  ) {
    return {
      isValid: false,
      mensage: " O campo product related é obrigatorio e deve ser um array",
    };
  }
...
route.post("/productRegister", (req: Request, res: Response) => {
  ...
  if (validBody.isValid) {
    ...
    res.json({
      mensage:
        "Cliente nao cadastrado corpo invalido, te acorda e coloca o dado certo",
    });
  }
});
